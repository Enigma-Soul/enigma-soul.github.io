...
- **用途**：
  快速增删改查（key 为 int 时相当于 array 但占用内存更少）
- **核心思想**：
  映射关系 一个好的 hash 函数所 hash 出的 key 较离散（当然也不能太离散） 较离散的 hash 函数会减少哈希冲突 以此减少时间复杂度（多多少少会有点的）<br>
  #### 负载因子
    哈希表的空间占有率 即
...
1. **边界条件**：无
2. **常见坑点**：唯一性 `key` 记得处理 `哈希冲突` ~~否则等着 WA 吧~~
